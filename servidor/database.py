...
def insert_data(origen, tabla, raw_line):
    """Inserta en ingestion_log y en la tabla dimensional correspondiente."""
    conn = get_connection()
    cur = conn.cursor()

    try:
        parts = raw_line.strip().split(",")
        contenido = json.dumps(parts)

        cur.execute(
            "INSERT INTO ingestion_log (origen, tabla_origen, contenido) VALUES (%s, %s, %s)",
            (origen, tabla, contenido),
        )
        ...
        if tabla == "customers" and len(parts) >= 5:
            ...
            cur.execute(
                ...
                (parts[0], parts[1], int(float(parts[2]) if parts[2] else 0),
            ...
            cur.execute(
                ...
                (parts[0], int(parts[1]), parts[2], parts[3],
                 float(parts[4]) if parts[4] else 0,
